...
from math_local.mathFunctions import fnGaussFit, fnGauss
...
scienceInputFolder = os.path.abspath('{}/reduced/{}/uves_obs_scired/'.format(WorkPath, settings.dataFolder))
        ...
        if ccfWriteFileName in ccfProcessedList:
            ...
            if nOrders == 1:
                ...
                for order in np.arange(nOrders):
                    ...
                    rangeCCF[order], nLines = zip(
                        *[fnComputeCCF(scienceData[order], waveData[order], maskData, testRV) for testRV in
                          rangeRV])
            ...
            if nOrders == 1:
                ...
                for order in np.arange(nOrders):
                    ...
                    A[order], mean[order], FWHM[order], B[order] = fnGaussFit(rangeRV, rangeCCF[order],
                                                                              GaussParamsInitGuess=[
                                                                                  max(rangeCCF[order]) - min(
                                                                                      rangeCCF[order]),
                                                                                  settings.guessRV -
